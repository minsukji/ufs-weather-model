    ...
    for j in range(len(cases[i])):
      ...
      if any(e in case for e in ['thr','mpi','dcp','rst']):
        ...
        if not bj['bld_set'] or not any(std in e for e in bj['bld_set']):
          ...
          bj['include'].append({'bld_set': std, 'name': test, 'case': std[-3:]})
        aj = {'test_set': case, 'name': test, 'case': case[-3:], 'artifact': std}
      else:
        bj['bld_set'].append(case)
        bj['include'].append({'bld_set': case, 'name': test, 'case': case[-3:]})
        aj = {'test_set': case, 'name': test, 'case': case[-3:], 'artifact': case}
